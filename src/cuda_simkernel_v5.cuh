...
__device__ PokerHandType getHandType_v5(const uint8_t* hand) {
...
__global__ void simKernel_v5c(curandState* states, uint64_t* globalCounts, uint32_t handsPerThread) {
...
#pragma unroll
    for (int j = 0; j < 48; ++j) {
      PokerHandType type = getHandType_v5(myDeck + j);
...
__global__ void simKernel_v5x(xoro_state128* states, uint64_t* globalCounts, uint32_t handsPerThread) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename F> DeviceConfig getDeviceConfig_v5(int maxBlockSize, F func) {
  DeviceConfig config;
  int          device;
...
void simRunCUDA_v5(SimulationResult& result) {
  ...
  if (result.cudaRngMethod == RNGMethod::CURAND) {
    config = getDeviceConfig_v5(result.cudaBlockSize, &simKernel_v5c);
  } else {
    config = getDeviceConfig_v5(result.cudaBlockSize, &simKernel_v5x);
  }
  ...
  try {
    ...
    if (result.cudaRngMethod == RNGMethod::CURAND) {
      simKernel_v5c<<<grid, block, sharedMemSize>>>(d_curandStates, d_counts, handsPerThread);
    } else {
      simKernel_v5x<<<grid, block, sharedMemSize>>>(d_xoroStates, d_counts, handsPerThread);
    }
    CHECK_CUDA_ERROR(cudaGetLastError());
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
