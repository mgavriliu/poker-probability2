...
// Version 2:
//  - simple card representation by uint8_t (0..51)
//  - per thread deck in shared memory for efficient shuffling

__device__ PokerHandType getHandType_v2(const uint8_t* hand) {
...
__global__ void simKernel_v2c(curandState* states, uint64_t* globalCounts, uint32_t handsPerThread) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;

  extern __shared__ unsigned char shared_mem[];
  uint8_t*                        decks  = (uint8_t*)shared_mem;
  uint8_t*                        myDeck = &decks[threadIdx.x * 52];
  //__syncthreads();
...
#pragma unroll
    for (int j = 0; j < 48; ++j) {
      PokerHandType type = getHandType_v2(myDeck + j);
...
__global__ void simKernel_v2x(xoro_state128* states, uint64_t* globalCounts, uint32_t handsPerThread) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename F> DeviceConfig getDeviceConfig_v2(int maxBlockSize, F func) {
  DeviceConfig config;
  int          device;
...
void simRunCUDA_v2(SimulationResult& result) {
  CUDA_DEBUG("Initializing CUDA calculation");

  DeviceConfig config;
  if (result.cudaRngMethod == RNGMethod::CURAND) {
    config = getDeviceConfig_v2(result.cudaBlockSize, &simKernel_v2c);
  } else {
    config = getDeviceConfig_v2(result.cudaBlockSize, &simKernel_v2x);
  }
  ...
  try {
    ...
    if (result.cudaRngMethod == RNGMethod::CURAND) {
      simKernel_v2c<<<grid, block, sharedMemSize>>>(d_curandStates, d_counts, handsPerThread);
    } else {
      simKernel_v2x<<<grid, block, sharedMemSize>>>(d_xoroStates, d_counts, handsPerThread);
    }
    CHECK_CUDA_ERROR(cudaGetLastError());
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
