...
// Version 3:
//  - hand/cards dynamically converted to representation using 4x uint16_t
//  - new representation allows more direct hand type evaluation
//  - shared memory for per-thread deck storage speeds up shuffling

__device__ PokerHandType getHandType_v3(const uint8_t* hand) {
  CudaCard cards[5] = {
      getCudaCard(hand[0]), getCudaCard(hand[1]), getCudaCard(hand[2]), getCudaCard(hand[3]), getCudaCard(hand[4])};
  uint16_t hv0 = cards[0].v0 + cards[1].v0 + cards[2].v0 + cards[3].v0 + cards[4].v0;
  uint16_t hv1 = cards[0].v1 + cards[1].v1 + cards[2].v1 + cards[3].v1 + cards[4].v1;
  uint16_t hv2 = cards[0].v2 + cards[1].v2 + cards[2].v2 + cards[3].v2 + cards[4].v2;
  uint16_t hv3 = cards[0].v3 + cards[1].v3 + cards[2].v3 + cards[3].v3 + cards[4].v3;
...
__global__ void simKernel_v3c(curandState* states, uint64_t* globalCounts, uint32_t handsPerThread) {
...
#pragma unroll
    for (int j = 0; j < 48; ++j) {
      PokerHandType type = getHandType_v3(myDeck + j);
...
__global__ void simKernel_v3x(xoro_state128* states, uint64_t* globalCounts, uint32_t handsPerThread) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename F> DeviceConfig getDeviceConfig_v3(int maxBlockSize, F func) {
  DeviceConfig config;
  int          device;
...
void simRunCUDA_v3(SimulationResult& result) {
  ...
  if (result.cudaRngMethod == RNGMethod::CURAND) {
    config = getDeviceConfig_v3(result.cudaBlockSize, &simKernel_v3c);
  } else {
    config = getDeviceConfig_v3(result.cudaBlockSize, &simKernel_v3x);
  }
  ...
  try {
    ...
    if (result.cudaRngMethod == RNGMethod::CURAND) {
      simKernel_v3c<<<grid, block, sharedMemSize>>>(d_curandStates, d_counts, handsPerThread);
    } else {
      simKernel_v3x<<<grid, block, sharedMemSize>>>(d_xoroStates, d_counts, handsPerThread);
    }
    CHECK_CUDA_ERROR(cudaGetLastError());
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
