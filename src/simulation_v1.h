...
class SimulationV1 : public Simulation {
 public:
  SimulationV1(const SimulationResult& params) { this->result = params; }
  ~SimulationV1() = default;

  virtual std::string id() override { return "cpu:v1"; }
  ...
  void run() {
    ...
    std::cout << "\nStarted " << this->id() << " with " << formatNumber(this->result.numHands) << " hands..."
              << std::endl;
...
#pragma unroll
      ...
      for (int k = 0; k < 48; ++k) {
        PokerHand hand = PokerHand(deck + k, 5);  // Constructor determines the hand type
        localCounts.counts[static_cast<uint64_t>(hand.getHandType())]++;
      }
    }
    return localCounts;
  }
